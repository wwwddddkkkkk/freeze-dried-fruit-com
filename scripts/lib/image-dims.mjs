// Read intrinsic pixel dimensions from local SVG / JPG / PNG / WebP files at build
...
// WebP is a RIFF container. Dimensions may live in one of three image chunks:
// VP8X (extended), VP8 (lossy), or VP8L (lossless). Walk chunks instead of
// assuming the image chunk is first because metadata chunks may precede it.
function readWebpDimensions(buf) {
  ...
  while (offset + 8 <= buf.length) {
    ...
    if (
      type === "VP8 " && size >= 10 &&
      buf[data + 3] === 0x9d && buf[data + 4] === 0x01 && buf[data + 5] === 0x2a
    ) {
      return {
...
export function readImageDimensions(absPath) {
  try {
    ...
    if (lower.endsWith(".webp")) return readWebpDimensions(buf);
    return null;
  } catch {
    return null;
  }
