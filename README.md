...
Editorial site for the freeze-dried fruit field guide. Markdown articles and JSON config compile to static HTML and deploy through Cloudflare Workers Builds. The News Wire on the homepage is auto-refreshed by a GitHub Action.
...
To publish, commit the new `.md` and push. Cloudflare Workers Builds rebuilds and deploys `main` automatically.
...
The production Worker is `freeze-dried-fruit-com` and is connected to the
GitHub repository `wwwddddkkkkk/freeze-dried-fruit-com`.
...
Cloudflare watches `main` and rebuilds on every push — including automated
commits from the News Wire workflow. GitHub Actions does not deploy the site;
it only refreshes and commits the feed.
...
| Trigger                                 | Effect                                                                |
| --------------------------------------- | --------------------------------------------------------------------- |
| `git push` to `main`                    | Cloudflare Workers Builds rebuilds and deploys production             |
...
```
content/
  articles/         editorial Markdown articles (the main content source)
...
wrangler.jsonc      Cloudflare Worker name and static asset directory
scripts/
